...
function decode(row:Row):Entry{return{...row.data,id:row.id,revision:row.revision,createdAt:row.created_at,updatedAt:row.updated_at}}
...
export async function isPublicAsset(id:string){for(let offset=0;;){const rows=await rest('portfolio_entry_assets?select=entry_id,portfolio_entries!inner(status,data)'+filter('asset_id',id)+'&portfolio_entries.status=eq.published&order=entry_id.asc&limit=100&offset='+offset);if(!rows.length)return false;if(rows.some((row:{portfolio_entries:{data:EntryInput}})=>row.portfolio_entries?.data&&assetReferences(row.portfolio_entries.data).includes(id)))return true;offset+=rows.length}}
