...
export function renderMarkdown(markdown:string){const toc:{id:string;label:string}[]=[];const ids=new Set<string>();const parser=new Marked({gfm:true,breaks:false});
 ...
 image({href,text,title}){const url=safeLink(href,true);if(!url)return '<span>'+escape(text)+'</span>';return '<figure><img src="'+escape(url)+'" alt="'+escape(text)+'" loading="lazy"/>'+(title?'<figcaption>'+escape(title)+'</figcaption>':'')+'</figure>'}
 }});
